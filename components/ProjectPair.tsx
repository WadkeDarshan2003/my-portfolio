...
export const ProjectCard = ({ 
  ...
  const containerClasses = isPreview
    ...
    : "w-full md:w-1/2 h-[50vh] md:h-auto md:aspect-square lg:h-screen lg:aspect-auto relative flex flex-col overflow-hidden transition-all duration-500 ease-out group cursor-pointer border-b border-r border-slate-100 dark:border-white/[0.06]";
      ...
      <div className="flex-1 p-6 md:p-10 lg:p-14 flex flex-col justify-between relative z-20">
        ...
        <div className="transform transition-transform duration-500 translate-y-0 group-hover:-translate-y-1">
          <div className="flex items-center gap-3 mb-4">
             <span className="px-2.5 py-1 text-[10px] md:text-xs font-bold uppercase tracking-widest border border-slate-900/5 dark:border-white/10 rounded-md bg-white/50 dark:bg-white/5 text-slate-600 dark:text-neutral-400 backdrop-blur-sm">
               {project.category || "Category"}
             </span>
             <span className="hidden sm:flex px-2.5 py-1 text-[10px] md:text-xs font-bold uppercase tracking-widest border border-slate-900/5 dark:border-white/10 rounded-md bg-white/50 dark:bg-white/5 text-slate-600 dark:text-neutral-400 items-center gap-1 backdrop-blur-sm">
               <Clock size={12} /> {project.duration || "Duration"}
             </span>
          </div>
          <h3 className="text-3xl md:text-4xl lg:text-5xl font-serif text-slate-900 dark:text-white mb-2 leading-tight drop-shadow-sm dark:drop-shadow-none">
            {project.title || "Project Title"}
          </h3>
          {!isPreview && (
            <div className="flex items-center gap-2 text-xs md:text-sm font-medium text-slate-500 dark:text-neutral-500 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
        ...
        <div className="absolute inset-0 md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 w-full h-full md:w-[60%] md:h-[45%] opacity-10 md:opacity-100 transition-all duration-700 pointer-events-none md:pointer-events-auto z-10 flex items-center justify-center">
           <div className={`relative w-full h-full overflow-hidden shadow-2xl transition-all duration-700 rounded-none md:rounded-lg border border-white/20 dark:border-white/5 ${isHovered ? 'scale-105 shadow-[0_30px_60px_-12px_rgba(50,50,93,0.25)] dark:shadow-[0_0_50px_-12px_rgba(255,255,255,0.1)]' : 'scale-100'}`}>
             {project.image ? (
               ...
               <div className="w-full h-full bg-slate-200 dark:bg-neutral-900 flex items-center justify-center text-slate-400">No Image</div>
             )}
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.stack.slice(0, 3).map(tech => (
                    <span
                      key={tech} 
                      className="text-slate-600 dark:text-neutral-400 text-[10px] font-bold uppercase tracking-wider bg-slate-100/50 dark:bg-white/5 px-2 py-1 rounded border border-slate-200/50 dark:border-white/5"
                    >
                      {tech}
                    </span>
                  ))}
  ...
  return (
    <section className="flex flex-col md:flex-row w-full min-h-screen">
      ...
      {projectRight ? (
        ...
        <div className="hidden md:block w-1/2 bg-stone-50 dark:bg-[#020202] border-b border-slate-100 dark:border-white/[0.06]">
